...
void setup_nrnthreads_on_device(NrnThread* threads, int nthreads) {
    ...
    for (i = 0; i < nthreads; i++) {
        ...
        if (need_record && nt->nrec_param > 0)
        {
            ...
            acc_delete(nt->rec_mech_types, sizeof(int) * nparam);
            acc_delete(nt->rec_param_index, sizeof(int) * nparam);
            acc_delete(nt->is_rec_v, sizeof(bool) * nparam);
        ...
        if (has_non_v_gap)
        {
            ...
            acc_delete(nt->gap_src_mech_types, sizeof(int) * nparam);
            acc_delete(nt->gap_tgt_mech_types, sizeof(int) * nparam);
            acc_delete(nt->gap_src_param_idx, sizeof(int) * nparam);
            acc_delete(nt->gap_tgt_param_idx, sizeof(int) * nparam);
            acc_delete(nt->gap_src_is_v, sizeof(bool) * nparam);
            acc_delete(nt->gap_tgt_is_v, sizeof(bool) * nparam);
            ...
            delete[] nt->gap_src_is_v;
            nt->gap_src_is_v = NULL;
            delete[] nt->gap_tgt_is_v;
        ...
        if (is_training)
        {
            ...
            acc_delete(nt->w_mech_types, sizeof(int) * len);
            acc_delete(nt->w_idx, sizeof(int) * len);
            delete[] nt->w_mech_types;
            nt->w_mech_types = NULL;
            delete[] nt->w_idx;
            nt->w_idx = NULL;

            len = nt->ndelta_weights;
            ...
            #pragma acc parallel loop present(nt[0:1])
            for (int i = 0; i < nt->ndelta_weights; i++)
            {
            ...
            acc_delete(nt->dw_idx, sizeof(int) * len);
            acc_delete(nt->dw_mech_types, sizeof(int) * len);
            delete[] nt->dw_idx;
            nt->dw_idx = NULL;
            ...
            {
                len = nt->nlabel_params;
                d_idx = (int*)acc_copyin(nt->label_idx, sizeof(int) * len);
                acc_memcpy_to_device(&(d_nt->label_idx), &d_idx, sizeof(int*));

                d_mech_types = (int*)acc_copyin(nt->label_mech_types, sizeof(int) * len);
                acc_memcpy_to_device(&(d_nt->label_mech_types), &d_mech_types, sizeof(int*));

                d_ptr = (double**)acc_copyin(nt->label_ptrs, sizeof(double*) * len);
                acc_memcpy_to_device(&(d_nt->label_ptrs), &d_ptr, sizeof(double**));

                #pragma acc parallel loop present(nt[0:1])
                for (int i = 0; i < nt->nlabel_params; i++)
                {
                    mech_type = nt->label_mech_types[i];
                    ix = nt->label_idx[i];
                    nt->label_ptrs[i] = nt->_ml_list[mech_type]->data + ix;
                }

                acc_delete(nt->label_mech_types, sizeof(int) * len);
                acc_delete(nt->label_idx, sizeof(int) * len);
                delete[] nt->label_mech_types;
                nt->label_mech_types = NULL;
                delete[] nt->label_idx;
                nt->label_idx = NULL;

                printf("shared_weight:%d\n", shared_weight);
                printf("adam_optim:%d\n", adam_optim);
                printf("avg_train:%d\n", avg_train);
                if (shared_weight)
                {
                    d_idx = (int*)acc_copyin(nt->w_buffer_idx, sizeof(int) * nt->nweights);
                    acc_memcpy_to_device(&(d_nt->w_buffer_idx), &d_idx, sizeof(int*));
                    printf("nweight:%d\n", nt->nweights);
                    d_idx = (int*)acc_copyin(nt->dw_buffer_idx, sizeof(int) * nt->ndelta_weights);
                    acc_memcpy_to_device(&(d_nt->dw_buffer_idx), &d_idx, sizeof(int*));
                    printf("ndw:%d\n", nt->ndelta_weights);
                    double *d_buffer;
                    d_buffer = (double*)acc_copyin(nt->weights_buffer, sizeof(double) * nt->buffer_size);
                    acc_memcpy_to_device(&(d_nt->weights_buffer), &d_buffer, sizeof(double*));
                    printf("buffer_size:%d\n", nt->buffer_size);
                    d_buffer = (double*)acc_copyin(nt->grads_buffer, sizeof(double) * nt->buffer_size);
                    acc_memcpy_to_device(&(d_nt->grads_buffer), &d_buffer, sizeof(double*));
                    d_buffer = (double*)acc_copyin(nt->adam_m, sizeof(double) * nt->buffer_size);
                    acc_memcpy_to_device(&(d_nt->adam_m), &d_buffer, sizeof(double*));
                    d_buffer = (double*)acc_copyin(nt->adam_v, sizeof(double) * nt->buffer_size);
                    acc_memcpy_to_device(&(d_nt->adam_v), &d_buffer, sizeof(double*));
                    d_buffer = (double*)acc_copyin(nt->adam_m_hat, sizeof(double) * nt->buffer_size);
                    acc_memcpy_to_device(&(d_nt->adam_m_hat), &d_buffer, sizeof(double*));
                    d_buffer = (double*)acc_copyin(nt->adam_v_hat, sizeof(double) * nt->buffer_size);
                    acc_memcpy_to_device(&(d_nt->adam_v_hat), &d_buffer, sizeof(double*));
                }
            }
