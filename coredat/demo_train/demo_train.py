...
if __name__ == '__main__':
    ...
    for inet, net in enumerate(net_list):
        for mod_dict in net.seg2synlist.values():
            for synlist in mod_dict.values():
                synlist.reverse()
        net.is_train(200, 500, 1.)
        for i, layer in enumerate(net.train_layers):
            layer.weight = copy.deepcopy(net_list[0].train_layers[i].weight)
            if hasattr(layer, 'bias'):
                layer.bias = copy.deepcopy(net_list[0].train_layers[i].bias)
        net.load_weights()
        net.set_stim(x_train[inet], y_train[inet])
    ...
    for net in net_list:
        gap_lines = net.gen_gap_param_file(bbcore_folder)
        stim_lines = net.gen_stim_file(net.input.stims)
        label_lines = net.gen_label_file(net.output.synlist_out2inter, int(y_train.max() + 1))
        w_lines, dw_lines, buffer_size = net.gen_set_weight_file()

        out_gap_params += gap_lines
        out_stim_amp += stim_lines
        out_label_params += label_lines
        out_weight_params += w_lines
        out_delta_weight_params += dw_lines
    ...
    with open(os.path.join(bbcore_folder, "label_params"), "w") as f:
        f.writelines(f"{len(out_label_params):d}\n")
        f.writelines(out_label_params)
    with open(os.path.join(bbcore_folder, "weights"), "w") as f:
        f.writelines(f"{len(out_weight_params):d} {buffer_size:d}\n")
        f.writelines(out_weight_params)
    with open(os.path.join(bbcore_folder, "delta_weights"), "w") as f:
        f.writelines(f"{len(out_delta_weight_params):d}\n")
        f.writelines(out_delta_weight_params)

    out_param2rec = net_list[0].gen_trainable_syn_params()
    train_epochs = args.epochs
    nimg = args.trainlen
    sim_time_per_img = 500 # 500ms
    sim_time_per_epoch = nimg / batch_size * sim_time_per_img # ms
    save_times = [int(i * sim_time_per_epoch + 20) for i in range(1, train_epochs + 1)]
    line1 = f"{len(out_param2rec):d} {train_epochs:d} "
    for t in save_times:
        line1 += (str(t) + " ")
    line1 += "\n"
